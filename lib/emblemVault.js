...
export async function remoteMintSignature({ tokenId, signature }) {
  const r = await ev(`${V2}/mint-curated`, {
    ...
    // Emblem's curated mint uses `buyWithQuote` (the SDK's canonical method);
    // `buyWithSignedPrice` is not a recognized method and returns signedByCreator:false.
    body: { method: 'buyWithQuote', tokenId, signature, chainId: '1' },
  });
  if (r?.success === false) {
