...
import { resolveCuratedAsset, lookupBundled } from './curatedAssets';
...
export async function vaultStatus(tokenId, { nudge = true } = {}) {
  ...
  // PepeCheck's bulk indexer passes nudge:false to stay inside the cron budget.
  if (nudge) {
  ...
  // One meta read serves both the BTC-address recovery and the wrong-collection
  // check below.
  const meta = await ev(`${V2}/meta/${tokenId}`, { timeout: 20000, label: 'reading the vault' }).catch(() => null);

  // Wrong-collection detection: a vault created into the wrong `select`
  // collection (e.g. PEPECASH, a Rare Pepe, vaulted into Fake Rares) fails
  // `allowed()` and will NEVER mint — even after it loads. Compare the vault's
  // recorded collection against the asset's allow-list entry so the UI can say
  // "recreate this vault" instead of "wait for Emblem".
  const targetAssetName = String(meta?.targetAsset?.name || '').toUpperCase();
  const recordedProject = meta?.targetContract?.name || meta?.targetAsset?.projectName || '';
  const expectedProject = targetAssetName ? (lookupBundled(targetAssetName)?.projectName || '') : '';
  const mismatch = Boolean(recordedProject && expectedProject && recordedProject !== expectedProject);
  ...
  if (!loaded && meta) {
    try {
      const addrs = meta?.addresses || meta?.ownershipInfo?.balances || [];
      btcAddress = addrs.find((a) => a?.coin === 'BTC' && a?.address)?.address || '';
      if (btcAddress) {
  ...
  return { values, loaded, mismatch, asset: targetAssetName, recordedProject, expectedProject, raw: res, source, btcAddress };
}

// Vaults associated with an address. Emblem's vault types (per the SDK):
//   created   — vaults this wallet created (wrap flow: resume/mint)
//   unclaimed — minted vaults this wallet holds, contents not yet claimed
//               (the SDK's own jump/migrate reports use this for "owned")
//   vaulted   — legacy/alternate owned-vault type
// The API paginates ({ data, pagination }), so request page 1 explicitly and
// walk pages if needed. For 'vaulted' requests (the unwrap flow) we merge
// 'unclaimed' + 'vaulted', deduped — 'vaulted' alone often returns [].
async function myVaultsOfType(address, vaultType) {
  const out = [];
  let page = 1, totalPages = 1;
  while (page <= totalPages && page <= 5) { // hard cap: 500 vaults is plenty for this UI
    const r = await ev(`${V2}/myvaults/${address}?vaultType=${vaultType}&page=${page}&limit=100`, { timeout: 20000, label: 'finding your vaults' });
    const chunk = Array.isArray(r) ? r : r?.data || [];
    out.push(...chunk);
    totalPages = r?.pagination?.totalPages || 1;
    if (Array.isArray(r)) break; // legacy unpaginated response: one shot has everything
    page++;
  }
  return out;
}
...
export async function myVaults(address, vaultType = 'created') {
  const types = vaultType === 'vaulted' ? ['unclaimed', 'vaulted'] : [vaultType];
  const lists = await Promise.all(types.map((t) => myVaultsOfType(address, t).catch(() => [])));
  const seen = new Set();
  const merged = [];
  for (const v of lists.flat()) {
    const tokenId = String(v?.tokenId ?? v?.id ?? '');
    if (!tokenId || seen.has(tokenId)) continue;
    seen.add(tokenId);
    merged.push({
      tokenId,
      asset: v.targetAsset?.name || v.name || v.assetName || '',
      collection: v.targetContract?.name || '',
