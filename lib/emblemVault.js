...
// EIP-55 checksum outbound addresses — good hygiene (matches the SDK, whose
// signer returns checksummed). Note: this was NOT the mint failure (Emblem
// normalizes stored addresses); the real fix was the buyWithSignedPrice method.
...
export async function vaultStatus(tokenId) {
  ...
  // Fallback: if Emblem's indexer shows nothing, confirm directly against
  // Counterparty using the vault's own BTC address (from its metadata).
  if (!values.length) {
    try {
  ...
  return { values, raw: res, source, btcAddress };
}
...
export async function remoteMintSignature({ tokenId, signature }) {
  const r = await ev(`${V2}/mint-curated`, {
    ...
    // The current SDK (evm-operations.ts) uses buyWithSignedPrice, NOT buyWithQuote.
