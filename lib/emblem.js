...
import { CONTRACT_TO_ID } from './collections';
  ...
  return memo(`emblem:vaults:${addr}`, 5 * 60 * 1000, async () => {
    ...
    try {
      ...
      for (const v of d) {
        ...
        const collection = CONTRACT_TO_ID[contract];
        if (!collection) continue; // not a tracked pepe collection
        ...
        rarePepe.push({ asset, image: ta.image || null, tokenId: v.tokenId, collection });
      }
