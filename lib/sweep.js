...
async function sweepDispensers(assetSet) {
  ...
  do {
    const url = `${CP}/dispensers?status=open&limit=500${cursor ? `&cursor=${cursor}` : ''}`;
    const d = await getJson(url, { accept: 'application/json' });
    for (const disp of d.result || []) {
      const asset = String(disp.asset || '').toUpperCase();
      if (!assetSet.has(asset) || !(disp.give_remaining > 0)) continue;
  ...
  return { floorSatsByAsset, dispensers, capped: Boolean(cursor) };
}
