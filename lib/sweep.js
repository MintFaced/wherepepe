...
// Deterministic hourly cache key. Each hour is a distinct Data Cache entry, so
// the snapshot refreshes at the top of every hour (aligned with the cron). No
// revalidateTag (which wipes same-request writes) and no read-side race.
function hourBucket() {
  return String(Math.floor(Date.now() / (60 * 60 * 1000)));
}

const cachedBuild = unstable_cache(
  async (_bucket) => buildSnapshot(),
  ['wrapped-snapshot-v3'],
  { revalidate: 2 * 60 * 60 },
);

export async function getWrappedSnapshot() {
  return cachedBuild(hourBucket());
}

// Reads: serve the cached snapshot. A cold hour bucket builds once (~7s) and is
// then warm for everyone; the hourly cron keeps it warm so reads rarely wait.
export async function peekWrappedSnapshot() {
  try {
    return await getWrappedSnapshot();
  } catch {
    return EMPTY_SNAPSHOT;
  }
}

// Cron: build (and cache) the current hour's snapshot.
