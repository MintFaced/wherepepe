...
async function buildSnapshot() {
  if (!process.env.OPENSEA_API_KEY) return EMPTY_SNAPSHOT;
  {
    ...
    // Each sweep degrades independently — a failure in one (e.g. an OpenSea
    // rate-limit on offers) must not wipe the others. A *total* failure throws
    // below so the empty result is NOT cached (it retries next call).
    const [nftRes, listRes, dispRes, offerRes, rates] = await Promise.all([
      enumerateNfts(assetSet).catch(() => ({ tokenToAsset: {}, nfts: 0, capped: false })),
      sweepListings().catch(() => ({ floorByToken: {}, listings: 0, capped: false })),
      sweepDispensers(assetSet).catch(() => ({ floorSatsByAsset: {}, divisible: new Set(), dispensers: 0, capped: false })),
      sweepOffers().catch(() => ({ offerByToken: {}, collectionOffer: 0, offers: 0, capped: false })),
      getRates().catch(() => ({ btcEth: null })),
    ]);

    // If we couldn't even enumerate the collection, treat as a transient total
    // failure: throw so unstable_cache doesn't persist an empty snapshot.
    if (nftRes.nfts === 0 && Object.keys(listRes.floorByToken).length === 0) {
      throw new Error('sweep produced no data (likely rate-limited) — not caching');
    }

    // Wrapped floor per asset (join listings → tokenId → asset). Also remember
    // which tokenId set the floor, for a direct OpenSea buy link.
    const wrappedByAsset = {};
    const wrappedTokenByAsset = {};
    for (const [tokenId, eth] of Object.entries(listRes.floorByToken)) {
      const asset = nftRes.tokenToAsset[tokenId];
      if (!asset) continue;
      if (wrappedByAsset[asset] == null || eth < wrappedByAsset[asset]) {
        wrappedByAsset[asset] = eth;
        wrappedTokenByAsset[asset] = tokenId;
      }
    }
    ...
    // Highest offer per asset — item offers (mapped by tokenId) and, for cards
    // in the collection, the collection-wide offer as a baseline.
    const collectionAssets = new Set(Object.values(nftRes.tokenToAsset));
    const offerByAsset = {};
    ...
    const assets = new Set([
      ...Object.keys(wrappedByAsset),
      ...Object.keys(nativeByAsset),
      ...Object.keys(offerByAsset),
    ]);
    for (const a of assets) {
      ...
      const colOffer = collectionAssets.has(a) ? offerRes.collectionOffer : 0;
      const highestOfferEth = Math.max(itemOffer, colOffer) || null;
      if (highestOfferEth != null) assetsWithOffer += 1;
      byAsset[a] = { wrappedFloorEth, nativeFloorEth, cheaper, savingsPct, highestOfferEth, wrappedTokenId: wrappedTokenByAsset[a] ?? null };
    }
    ...
    return {
      ...
      stats: {
        ...
        assetsWithOffer,
        collectionOfferEth: offerRes.collectionOffer || null,
