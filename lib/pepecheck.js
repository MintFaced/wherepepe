...
export async function staleVaultTokenIds(limit = 20) {
  ...
  return sql`
    ...
    LEFT JOIN pc_vaults v ON v.token_id = l.token_id
    WHERE l.active AND l.card IS NOT NULL
      AND (v.token_id IS NULL OR v.verified_at < now() - interval '6 hours')
    ORDER BY v.verified_at ASC NULLS FIRST LIMIT ${limit}`;
}

// Tokens listed but not yet named (card IS NULL) for one contract.
export async function unnamedListingTokenIds(contract, limit = 4000) {
  if (!sql) return new Set();
  const r = await sql`SELECT token_id FROM pc_listings WHERE contract = ${contract} AND active AND card IS NULL LIMIT ${limit}`;
  return new Set(r.map((x) => x.token_id));
}

export async function setListingCard(tokenId, card, collection) {
  if (!sql) return;
  await sql`UPDATE pc_listings SET card = ${card}, collection = coalesce(${collection}, collection) WHERE token_id = ${tokenId}`;
}

// Contract that a token is listed under (for correct OpenSea links).
export async function listingContractFor(tokenId) {
  if (!sql) return null;
  const r = await sql`SELECT contract FROM pc_listings WHERE token_id = ${tokenId} ORDER BY active DESC, updated_at DESC LIMIT 1`;
...
// Bulk NFT names for a contract: yields { identifier, name } 200 at a time.
// One call names 200 tokens vs one call per token — this is how the whole
// curated index gets named in a couple of cron runs.
export async function* osNftPages(contract, maxPages = 20) {
  let next = '', page = 0;
  do {
    const d = await os(`/chain/ethereum/contract/${contract}/nfts?limit=200${next ? `&next=${next}` : ''}`).catch(() => null);
    if (!d) return;
    for (const n of d.nfts || []) yield { identifier: String(n.identifier), name: n.name || '' };
    next = d.next || '';
    page++;
